...
namespace CascadeDropdown.Models
{
    public class AdminController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult StateSuccess(StateViewModel stateViewModel)
        {
            List<Country> allCountry = new List<Country>();
            using (DatabaseContext dbContext = new DatabaseContext())
            {
                allCountry = dbContext.Country.OrderBy(a => a.CountryName).ToList();
                _State.StateName = stateViewModel.StateName;
                _State.CountryId = stateViewModel.CountryId;
                dbContext.State.Add(_State);
                dbContext.SaveChanges();
                ModelState.Clear();
        ...
        public ActionResult StateList()
        {
            using (DatabaseContext dbcontext = new DatabaseContext())
            {
                return View(dbcontext.State.ToList());
            }
